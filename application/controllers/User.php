...
class User extends CI_Controller
{
    ...
    public function edit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($legal_documents) {
                if ($_FILES['legal_documents']['size'] > 2097152) {
                    echo "<script>alert('File is too large. Maximum file size is 2MB.'); window.location.href = '" . base_url('user/edit') . "'</script>";
                }
            ...
            if ($chart) {
                if ($_FILES['organizational_chart']['size'] > 2097152) {
                    echo "<script>alert('File is too large. Maximum file size is 2MB.'); window.location.href = '" . base_url('user/edit') . "'</script>";
                }
            ...
            $this->db->set('description_company' , $description_company);
            $this->db->set('weblink' , $weblink);
    ...
    public function cetak_certificate($id)
    {
        ...
        if($cek->num_rows() > 0){
            ...
            $data['count_certificate'] = count($this->db->where('id_user', $this->session->userdata('id'))->get('certificate_user')->result_array()) + 1;
