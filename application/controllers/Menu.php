...
class Menu extends CI_Controller
{
    ...
    public function delete_admin_certification_manager()
    {
        $delete = $this->db->where('id', $this->input->get('id'))->delete('user');
        $msg['status'] = 'tidak';
        if ($delete) {
    ...
    public function admin_certification_manager()
    {
        $data['title'] = 'Admin Cerficiation Manager';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $data['menu'] = $this->db->get('user_menu')->result();
        $data['data'] = $this->db->select('user.id, user.name, user.email, user.password_default, user.date_created')->from('user')->join('user_role', 'user_role.id = user.role_id')->where('user.role_id', '10')->get()->result();
        $data['hak_akses_menu'] = $this->db->select('user_menu.id, user_menu.menu, user_access_menu.id as user_access_menu_id')->from('user_access_menu')->where('user_access_menu.role_id', '10')->join('user_menu', 'user_menu.id = user_access_menu.menu_id')->get()->result();
        $this->form_validation->set_rules('menu', 'Menu', 'required');

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('menu/admin_certification_manager', $data);
        $this->load->view('templates/footer');
    }

    public function store_admin_certification_manager()
    {
        $generate = $this->generateRandomString();
        $this->db->insert('user', [
            'name' => $this->input->post('name'),
         ...
         echo "<script>alert('Admin certification manager bernama " . $this->input->post('name') . " berhasil di tambah'); window.location.href = '" . base_url('menu/admin_certification_manager') . "'</script>";
    
       
    }

    public function update_admin_certification_Manager()
    {
        $this->db->where('id', $this->input->post('id'))->update('user', [
            'name' => $this->input->post('name'),
            'email' => $this->input->post('email')
        ]);
        echo "<script>alert('Admin Certification Manager bernama " . $this->input->post('name') . " berhasil di edit'); window.location.href = '" . base_url('menu/admin_certification_manager') . "'</script>";
    }
    
    public function add_hak_akses_admin_certification_manager()
    {
        $cek = $this->db->where('role_id', '10')->where('menu_id', $this->input->post('menu'))->get('user_access_menu')->result();
        if (count($cek) > 0) {
            echo "<script>alert('Menu ini telah dipilih sebelumnya'); window.location.href = '" . base_url('menu/admin_certification_manager') . "'</script>";
        } else {
            $this->db->insert('user_access_menu', [
                'role_id' => '10',
                'menu_id' => $this->input->post('menu')
            ]);
            echo "<script>alert('Berhasil tambah hak akses menu admin finance'); window.location.href = '" . base_url('menu/admin_certification_manager') . "'</script>";
        }
    }

    public function delete_hak_akses_admin_certification_manager()
    {
        $delete = $this->db->where('id', $this->input->get('id'))->delete('user_access_menu');
        $msg['status'] = 'tidak';
        if ($delete) {
