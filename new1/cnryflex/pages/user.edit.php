...
if(!$user = mysqli_fetch_assoc($core->data->query("SELECT * FROM `tb_users` WHERE `idUser` = '$get'"))){
    $core->toast('warning', 'Data not found!', 'pages/user.list');
} ?>

<section class="center">
    <form class="edit validate cx-form cx-lg" action="<?php echo urlFlex ?>system/process/user/edit" method="POST" enctype="multipart/form-data">
        <?php $core->formTitle('edit', 'Edit User') ?>
        ...
        <div class="photo">
            ...
            <label for="theImg" class="profile"><img src="<?php echo urlFlex ?>assets/img/profile/<?php echo (!empty($user['img'])) ? $user['img'] : '00-no-image.png' ?>"></label><input type="file" accept="image/png, image/jpeg" name="profile" id="theImg" class="photo1"/><input id="temporary" disabled>
        </div>

        <div class="obj">
            <label>Name</label>
            <i class="micon">person</i>
            <input type="text" name="name" class="with-icon" spellcheck="false" placeholder="Your full name" autocomplete="off" value="<?php echo $user['name'] ?>" required/>
        </div>

        <div class="obj two">
            <label>Role</label>
            <i class="micon">business</i>
            <select name="role" class="with-icon getSelect2" required>
                <option value="" selected disabled>Select role</option>
                <?php $sql = $core->data->query("SELECT idRole, `name` FROM tb_roles");
                while($row = mysqli_fetch_assoc($sql)){
                    if($row['idRole'] == $user['idRole']){ ?>
                    <option value="<?php echo $row['idRole'] ?>" selected><?php echo $row['name'] ?></option>
                    <?php }
                    if($row['idRole'] != $user['idRole']){ ?>
        ...
        <div class="obj three">
            ...
            <select class="with-icon getSelect2" name="lang" required>
                ...
                <option value="english" <?php if($user['lang'] == 'english') echo 'selected' ?>>English</option>
                <option value="bahasa" <?php if($user['lang'] == 'bahasa') echo 'selected' ?>>Bahasa</option>
            </select>
        </div>
        ...
        <div class="obj">
            ...
            <textarea name="address" class="check" maxlength="250" spellcheck="false" placeholder="Address" required check-target="check-textarea"><?php echo $user['address'] ?></textarea>
        </div>
        ...
        <div class="obj three">
            ...
            <input type="text" name="phone" class="with-icon" spellcheck="false" required autocomplete="off" value="<?php echo $user['phone'] ?>" placeholder="Ex : 087871894990"/>
        </div>

        <div class="obj three switch">
            <label>Status</label>
            <input type="radio" id="switch1" name="status" value="granted" <?php echo ($user['status'] == 'granted') ? 'checked' : '' ?>><label class="switch-label first" for="switch1">Granted</label>

            <input type="radio" id="switch2" name="status" value="blocked" <?php echo ($user['status'] == 'blocked') ? 'checked' : '' ?>><label class="switch-label" for="switch2">Blocked</label>
        </div>
        ...
        <div class="group">
            ...
            <div class="obj" style="display: flex; align-content: flex-start; flex-wrap: wrap; justify-content: space-between">
                <?php $sql = $core->data->query("SELECT tb_menus.name, tb_menus.control, tb_menus.title, tb_menus.idMenu, tb_privileges.type FROM tb_menus LEFT JOIN tb_privileges ON tb_menus.name = tb_privileges.namePage AND tb_privileges.idUser = '{$user['idUser']}' ORDER BY tb_menus.order");

                while($privil = mysqli_fetch_assoc($sql)){
                    if($privil['control'] == 'y'){ ?>
                    <div class="group-check">
                        <input type="checkbox" name="page[]" id="<?php echo $privil['idMenu'] ?>" value="<?php echo $privil['name'] ?>" <?php echo (empty($privil['type'])) ? '' : 'checked' ?>><label class="checkbox" for="<?php echo $privil['idMenu'] ?>"><?php echo $privil['title'] ?></label>
        ...
        <div class="group">
            ...
            <div class="obj two group--desc">
                <span>Last Time Change Password</span>
                <span><?php echo (!$user['fgtDate']) ? 'Has never been' : date('d F Y', strtotime($user['fgtDate'])) ?></span>
            </div>
            ...
            <div class="obj two">
                ...
                <input type="password" name="pass1" class="with-icon check" minlength="8" check-target="checkPass" passToggle placeholder="Your new password"/>
            </div>

            <div class="obj two">
                <label>Re-type New Password</label>
                <i class="micon">autorenew</i>
                <input type="password" name="pass2" class="with-icon" minlength="8" placeholder="Retype your password"/>
        ...
        <?php $core->modified($user['created'], $user['modified'], $user['idUser'] ) ?>
